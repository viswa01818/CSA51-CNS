...
int main() {
    char ciphertext[] = "GSV JFRXP YILDM ULC QFNKH LEVI GSV OZAB WLT";
    int top_n = 5;
    
    printf("Monoalphabetic Frequency Attack (Second Implementation)\n\n");
    printf("Ciphertext: %s\n", ciphertext);
    printf("Finding top %d possible plaintexts...\n\n", top_n);
    
    struct result { double score; char key[27]; char plaintext[1000]; } results[500];
    int count = 0;
